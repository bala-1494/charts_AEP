...
def configure_chart_form(df):
    """Displays an expander with a form to configure and add a new chart."""
    with st.expander("➕ Add a New Chart", expanded=False):
        with st.form(key='chart_form'):
            ...
            # Filter dataframe to get columns relevant to the selected asset type
            ...
            # Dynamic form fields based on chart type
            if chart_type == "Line Chart":
                chart_config['parameter'] = st.selectbox("Select Parameter to Plot", numeric_cols)
                ...
                chart_config['parameters'] = st.multiselect("Select Columns to Display", all_cols)
                ...
                chart_config['parameter'] = st.selectbox("Select Parameter", numeric_cols)
                c1, c2 = st.columns(2)
                chart_config['min_val'] = c1.number_input("Gauge Minimum Value", value=0)
                chart_config['max_val'] = c2.number_input("Gauge Maximum Value", value=100)
...
if menu_choice == "Upload JSON":
    ...
    if df.empty:
        ...
        # Display the form to add a new chart
        configure_chart_form(df)
